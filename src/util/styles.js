const styles = {
  container: "max-w-[1230px] w-full m-auto px-[15px]",
  borderDarkTop:
    "border-[1px] border-solid border-dark border-b-[0px] border-x-[0px]",
  borderDarkBottom:
    "border-[1px] border-solid border-dark border-t-[0px] border-x-[0px]",
  sectionMarginY: "my-[30px] sm:my-[60px] lg:my-[100px]",
};

const textStyles = {
  title: "text-[42px] font-normal leading-[53px]",
  title2: "text-[32px] font-normal leading-[53px]",
  text: "text-[22px] font-normal leading-[40px]",
  text2: "text-[18px] font-normal leading-[53px]",
  textSmall: "text-[16px] font-normal leading-[100%]",
};

export { styles, textStyles };
